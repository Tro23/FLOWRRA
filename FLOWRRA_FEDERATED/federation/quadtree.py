...
class SpatialPartition:
    ...
    def contains(self, pos: np.ndarray) -> bool:
        ...
        x, y = pos[0], pos[1]
        return (
            self.bounds_x[0] <= x < self.bounds_x[1]
            and self.bounds_y[0] <= y < self.bounds_y[1]
        )
    ...
    def get_neighbors(self, all_partitions: List["SpatialPartition"]) -> List[int]:
        ...
        for other in all_partitions:
            ...
            shares_x = (
                abs(self.bounds_x[1] - other.bounds_x[0]) < 1e-6
                or abs(self.bounds_x[0] - other.bounds_x[1]) < 1e-6
            )
            shares_y = (
                abs(self.bounds_y[1] - other.bounds_y[0]) < 1e-6
                or abs(self.bounds_y[0] - other.bounds_y[1]) < 1e-6
            )

            overlaps_x = not (
                self.bounds_x[1] <= other.bounds_x[0]
                or self.bounds_x[0] >= other.bounds_x[1]
            )
            overlaps_y = not (
                self.bounds_y[1] <= other.bounds_y[0]
                or self.bounds_y[0] >= other.bounds_y[1]
            )
...
class QuadtreePartitioner:
    ...
    def _initialize_grid(self):
        ...
        print(
            f"[Quadtree] Initialized {self.num_holons} partitions in {self.grid_size}x{self.grid_size} grid"
        )
