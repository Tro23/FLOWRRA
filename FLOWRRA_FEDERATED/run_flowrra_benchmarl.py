...
from flowrra_model import FlowrraModelConfig, register_flowrra_model, create_flowrra_model_config
...
def setup_experiment_config(name: str = "flowrra_experiment") -> ExperimentConfig:
    ...
    config.on_policy_collected_frames_per_batch = 6400  # 32 agents * 200 steps
    config.on_policy_n_minibatch_iters = 10

    # Evaluation
    config.evaluation = True
    config.evaluation_interval = 20  # Evaluate every 20 iterations
...
def setup_task(n_agents: int = 32, max_steps: int = 500) -> PettingZooTask:
    """
...
def compare_results(baseline_folder: Path, flowrra_folder: Path):
    ...
    try:
        ...
        if 'system/avg_integrity' in flowrra_df.columns:
            axes[1, 0].plot(flowrra_df['system/avg_integrity'], color='green', alpha=0.7)
        ...
        if 'system/avg_coherence' in flowrra_df.columns:
            axes[1, 1].plot(flowrra_df['system/avg_coherence'], color='purple', alpha=0.7)
        ...
        if 'system/avg_integrity' in flowrra_df.columns:
            print(f"  Final integrity: {flowrra_df['system/avg_integrity'].iloc[-1]:.3f}")
...
def main():
    ...
    parser.add_argument("--agents", type=int, default=32, help="Number of agents")
    parser.add_argument("--holons", type=int, default=4, help="Number of holons (FLOWRRA only)")
    parser.add_argument("--iters", type=int, default=500, help="Training iterations")
    parser.add_argument("--no-sensor-fusion", action="store_true", help="Disable sensor fusion")
    ...
    task = setup_task(n_agents=args.agents, max_steps=500)
