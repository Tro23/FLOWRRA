...
class NodePositionND:
    ...
    def apply_rotation_action(self, action: int):
        ...
        if action == 0:  # Azimuth left
            self.azimuth_idx = (self.azimuth_idx - int(self.rotation_speed)) % self.azimuth_steps
    ...
    def rotate_towards_target(self):
        ...
        if self.azimuth_idx != self.target_azimuth_idx:
            diff = (self.target_azimuth_idx - self.azimuth_idx + self.azimuth_steps) % self.azimuth_steps
        ...
        if self.dimensions == 3 and self.elevation_idx != self.target_elevation_idx:
            if self.elevation_idx < self.target_elevation_idx:
                self.elevation_idx = min(self.elevation_idx + 1, self.target_elevation_idx)
            else:
                self.elevation_idx = max(self.elevation_idx - 1, self.target_elevation_idx)
    ...
    def get_state_vector(self, local_repulsion_grid: np.ndarray, 
                        node_detections: List[Dict], 
                        obstacle_detections: List[Dict],
                        max_neighbors: int = 5,
                        max_obstacles: int = 5) -> np.ndarray:
        """
        ...
        node_det_sorted = sorted(node_detections, key=lambda x: x['distance'])[:max_neighbors]
        obs_det_sorted = sorted(obstacle_detections, key=lambda x: x['distance'])[:max_obstacles]
