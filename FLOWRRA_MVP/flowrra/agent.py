...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        h_i = h.unsqueeze(2).expand(B, N, N, self.n_heads, self.out_features)  # [B, N, N, n_heads, out]
        h_j = h.unsqueeze(1).expand(B, N, N, self.n_heads, self.out_features)  # [B, N, N, n_heads, out]
