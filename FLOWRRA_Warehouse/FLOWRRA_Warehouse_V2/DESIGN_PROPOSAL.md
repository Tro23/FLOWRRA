...
## Decisions locked, 14 September

| question | decision | why |
|---|---|---|
| conv type | **gated 3D**, not graph conv | a graph conv discards Euclidean geometry; a gated 3D conv keeps topology (mask channel) AND space (array). Unregistered obstacles arrive in coordinates, not in the graph |
| gating | **mandatory** | measured 9x signal attenuation without it: a 3x3x3 kernel divides by 27 while only ~2.9 of those cells are track |
| edge features | **build the layer** | path conflict is a property of a PAIR; encoding it twice, once per node, is a worse representation of the same fact. `edge_feature_dim=0` must remain a working identity path so it stays ablatable |
| adjacency | **graph distance** | fourth and last phantom-pair instance, and the only one that corrupted a TOPOLOGY rather than a number |
| global entropy | **deferred** | needs `Z` from the shared field. Gibbs dims drop 3 -> 2 for now |
| `T` in the dead-fleet benchmark | `urgency = orders_remaining / max(1, steps_remaining)`, `T = 1/(1+urgency)` | bounded in (0,1], becomes order-queue depth unchanged in continuous mode |
| normalisation | **LayerNorm, plus separate encoders** | LayerNorm normalises AFTER the layer; the 231-vs-42 imbalance happens at its input. Separate encoders are what actually fix drowning |
| wait state | explicit `waiting_nodes`, VDA-reported | `ray_hit_waiting` cannot exist until a peer's wait is visible. Check the VDA 5050 spec for the existing state name |

### What the density volume actually is

Not a cube and not a ball. A **geodesic ball** `B(v, 5)` under the GRAPH metric,
which on a degree-~2.27 corridor graph is a thin skeleton of corridor arms: a
line segment mid-aisle, a cross at a junction, a T at an intersection, a stub at
a dead end. About 35 live cells of 1,331 -- **97% of the array is padding**.

Measured on a 344-cell map: **93 distinct mask shapes**, one covering 49% of all
cells. About **6.5 bits**, currently encoded in 231 float32 = **7,392 bits**.

The shape is itself the signal: it says what KIND of place the fleet is standing
in, which flattening destroys.

### Where the layout constraint belongs

| home | mechanism | status |
|---|---|---|
| input, as 231 raw dims | — | **wrong**: 1000x redundant, drowns the rays |
| architecture | gate the convolution | Phase 2 |
| output | `valid_action_masks`, -inf before argmax | **already exists** |
| loss / target | mask the bootstrap | **missing** -- §6 |

A loss penalty is the wrong instrument for the validity half: penalties are
soft, and the layout is absolute. Hard constraints belong in architecture and
masking, where violation is impossible.

---

## Suggested order

**PHASE 1 -- COMPLETE, 14 September.** All three behind flags defaulting to
current behaviour, so the baseline has not moved:

- `get_local_volume()` returns the 2-channel volume; mask no longer multiplied
  into R. Verified that `mask/(1+R)` rebuilt from the volume reproduces the flat
  vector exactly, and that unreachable and contested cells become disjoint sets.
- `_build_adjacency_graph()` on graph distance, flag `proximity.adjacency_metric`.
  Widens the proximity index to `interaction_radius`, which also widens where
  `mean_peer_gap` is censored -- that metric is not comparable across the change.
- Ray transform `d/(d+c)`, flag `density.ray_transform`. Measured: 37 and 29
  cells of clear aisle both read exactly 1.0 under `clip25`; `smooth` gives
  0.822 and 0.784.

**PHASE 2** -- gated 3D conv + separate encoders + LayerNorm; waiting state
machine; ray semantics (18 dims); `T` and local entropy.
