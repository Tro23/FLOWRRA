...
class GNNAgent:
    ...
    def __init__(
        ...
        # Masked Q-values from the most recent choose_actions() call, or None
        # if no forward pass ran. Read by core_warehouse's tabu veto.
    ...
    def choose_actions(
        ...
        if len(greedy_indices) > 0:
            ...
            with torch.no_grad():
                ...
                # ADDED 2026-09-13. Publish the masked Q-values so the
                # orchestrator can ask for the network's SECOND choice when it
                # has to veto the first (tabu masking in core_warehouse.step()).
                # Previously that veto fell back to random.choice over
                # non-tabu actions, discarding a ranking the network had already
                # produced and that was already sitting in this local.
                #
                # Masked, so structurally invalid actions are -inf here and can
                # never win a fallback either. Rows for fleets that EXPLORED
                # this step still carry a valid ranking -- exploration replaced
                # the chosen action, not the network's opinion about the rest.
                self.last_q_values = q_values_np

            self.policy_net.train()
        else:
            # No forward pass ran this step, so there is no ranking to offer.
            # The consumer must treat None as "fall back to the goal gradient".
            self.last_q_values = None
