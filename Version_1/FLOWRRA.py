...
class Flowrra:
    ...
    class Environment_A:
        ...
        def _calculate_angle_idx(self, pos1, pos2):
            ...
            if angle < 0:
                angle += 360
            return int(round(angle * self.angle_steps / 360)) % self.angle_steps

        def step(self, rotation_substeps=4):
            """Updates the state of the nodes in the loop. 
            The rotation_substeps helps in increasing the fuildity 
            for the eye to catch up to the changes"""
    ...
    def _compute_reward(self, next_state_data, new_cells_visited_count, entropy_threshold):
        ...
        # Priority 2: Check if the new state breaks coherence (crosses entropy threshold).
            ...
            coherence_reward = -20.0 * (next_entropy - entropy_threshold)
        
        # Priority 3: If coherent, reward the agent for maintaining low entropy.
        else:
            coherence_reward = (entropy_threshold - next_entropy) * 2.0
        
        # --- NEW: Penalty for nodes being too close ---
        proximity_penalty = 0
        node_positions = [pos for (pos, angle) in next_state_data]
        if len(node_positions) > 1:
            ...
            if pair_count > 0:
                ...
                min_avg_distance = 3.0 # A configurable threshold
                if avg_distance < min_avg_distance:
                    proximity_penalty = -1.0 * (min_avg_distance - avg_distance) * 5.0 # A stronger penalty

        return exploration_reward + coherence_reward + proximity_penalty
